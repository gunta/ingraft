...
flowchart TB
  ...
  subgraph Embed["Embed in workspace (offline, native tools)"]
    direction TB
    E1["git subtree (raw)"]
    E2["vendor-subtree (this project)"]
    E3["Vendir"]
    E4["git submodule"]
    E5["git clone"]
...
| **Git-subtree wrappers**       | **vendor-subtree** (this project), **Vendir** | Ergonomic CLI over `git subtree` + hygiene |
...
### 2. vendor-subtree (this project)

**npm**: [`vendor-subtree`](https://www.npmjs.com/package/vendor-subtree) · **Skill**: `vendor-subtree` (Claude Code)

Opinionated, multi-strategy CLI that wraps git subtree (and submodule / clone-ignore as fallbacks). Distributed both as a standalone npm package and as a Claude Code skill. Built on Effect-TS.

```bash
bunx vendor-subtree@latest                          # scan package.json, ask what to vendor
bunx vendor-subtree@latest init                     # set up vendoring
bunx vendor-subtree@latest add effect               # alias → Effect-TS/effect
bunx vendor-subtree@latest add effect --tag v3.21.2 # pin to a tag
bunx vendor-subtree@latest add effect --sync-package effect      # follow npm version
bunx vendor-subtree@latest add effect --strategy submodule       # alt strategy
bunx vendor-subtree@latest add effect --strategy clone-ignore    # alt strategy
bunx vendor-subtree@latest add effect --exclude-dir docs --max-file-size 1MB
bunx vendor-subtree@latest update --all
bunx vendor-subtree@latest doctor                   # diagnose hygiene
bunx vendor-subtree@latest refresh                  # regenerate agent docs, ignores
```

```mermaid
flowchart TB
  User["User intent<br/>(natural language)"] --> Skill["vendor-subtree<br/>(Claude Code)"]
  Skill --> Router{Intent}
  Router -->|"vendor X"| Add["vendor-subtree add"]
  Router -->|"scan deps"| Deps["vendor-subtree deps"]
  Router -->|"update all"| Update["vendor-subtree update --all"]
  Router -->|"check hygiene"| Doctor["vendor-subtree doctor"]
...
| ✅ Strengths                                                                                               | ❌ Weaknesses                        |
| ---------------------------------------------------------------------------------------------------------- | ------------------------------------ |
| **Three strategies**: subtree / submodule / clone-ignore                                                   | Newer project, small community       |
...
**Best for**: When you actively develop within the external repo. **Not recommended** for agent-driven workflows — initialization failures are a reliability killer. Effect, vendor-subtree, and Vendir all recommend subtree instead.
...
**Best for**: Wrapping CLIs / codifying conventions. **Best paired with a source-level approach** (subtree, vendor-subtree, OpenSrc). Strongest setups use both.

Examples: Anthropic's official `claude-api` skill, this repo's `vendor-subtree`, BTCA's `btca-cli` skill.
...
**Best for**: Letting agents drill into dependency internals without committing them. **Strong complement** to vendor-subtree / Vendir — vendor for repos you constantly touch, OpenSrc for "what does this random npm package do?" exploration.
...
| Feature                             | Raw `git subtree` | **vendor-subtree** (this project)     | **Vendir**        |
| ----------------------------------- | ----------------- | ------------------------------------- | ----------------- |
| **Strategies**                      | Subtree only      | Subtree + submodule + clone-ignore    | Subtree only      |
...
| **Agent skill distribution**        | ❌                | ✅ — `vendor-subtree`                 | ❌                |
...
- **vendor-subtree** — full-featured: multi-strategy, manifest scanning, monorepo-aware, ships as agent skill
...
| Feature                        | Raw Subtree | vendor-subtree | Vendir | Submodule | Clone | Skills   | Repomix   | Repobase   | mgrep        | Context7                      | RepoSwarm  | BTCA/3         | OpenSrc     |
| ------------------------------ | ----------- | -------------- | ------ | --------- | ----- | -------- | --------- | ---------- | ------------ | ----------------------------- | ---------- | -------------- | ----------- |
...
flowchart TB
  ...
  D1 -->|"Few core libs,<br/>used daily"| Vendor["✅ Vendor source<br/>(vendor-subtree / Vendir / raw subtree)"]
  ...
  Skills -->|"yes"| VS["+ vendor-subtree<br/>(Claude Code)"]
  Skills -->|"no"| Done["Done"]
...
| Your situation                                                             | Use                                            |
| -------------------------------------------------------------------------- | ---------------------------------------------- |
| Daily coding with one or a few key libraries                               | **vendor-subtree** (or Vendir, or raw subtree) |
| Want the agent to invoke the right vendoring command from natural language | **vendor-subtree** + Claude Code               |
...
flowchart TB
  subgraph Layer1["Layer 1 — Source depth (always available)"]
    direction LR
    L1A["vendor-subtree / Vendir<br/>for core libraries"]
...
1. **For serious agent-driven development, vendored source is the gold standard.** The Effect team's argument — agents read code better than docs — is architecturally sound. Whether you use raw `git subtree`, **vendor-subtree**, or **Vendir**, the underlying win is the same: navigable source in the workspace.

2. **The wrapper choice matters less than the wrapping choice.** Both vendor-subtree and Vendir are inspired by the Effect post. Pick vendor-subtree for multi-strategy + manifest scanning + monorepo support + skill integration. Pick Vendir for a minimal subtree-only wrapper with TOML config. Both beat running raw commands.
...
5. **Submodules remain a trap for agent workflows.** The initialization step is a reliability killer. Effect, vendor-subtree, and Vendir all reach the same conclusion: use subtree.
...
7. **The landscape is moving fast.** vendor-subtree, Vendir, mgrep, BTCA-3, and OpenSrc are all young or recently rebuilt. Expect more tooling. The fundamental insight — **agents work best with navigable source code in their workspace** — is unlikely to change.
...
- [vendor-subtree](https://www.npmjs.com/package/vendor-subtree) (this project)
