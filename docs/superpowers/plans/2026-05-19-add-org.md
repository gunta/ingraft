...
export const listOrgRepos = ({ owner }: { readonly owner: string }) =>
  Effect.gen(function* () {
    ...
    const result = yield* gh
      ...
      .pipe(
        Effect.catchAll((cause) => Effect.fail(new GitHubCliMissing({ cause })))
      )
  ...
  Effect.gen(function* () {
    ...
    return yield* Effect.try({
      try: () => JSON.parse(result.stdout),
        ...
        new GitHubOrgNotFound({ owner: args.slice(-1)[0] ?? "?" })
    })
  })
...
export const listVendored = (cwd: string) =>
  Effect.gen(function* () {
    ...
    const headResult = yield* git(["rev-parse", "HEAD"], { cwd }).pipe(
      Effect.option
    )
...
describe("default vendor prefix", () => {
  test("nests under owner for github shorthand", async () => {
    ...
    const name = await Effect.runPromise(
      inferRepoName("https://github.com/Effect-TS/effect.git")
    )
...
```ts
    ...
    const fallbackUser = { schemaVersion: 1, fetchedAt: new Date().toISOString(),
                            login: "", orgs: [] as ReadonlyArray<string> }
...
const visibilityFlag = Flag.choiceWithValue("visibility", [
  ...
  Flag.withDescription("Repository visibility filter."),
  Flag.optional
)
...
describe("dispatchAddOrg", () => {
  ...
  test("SelectAllFiltered selects every visible row", () => {
    const state = dispatchAddOrg(
      initial,
      AddOrgAction.SetLanguage({ values: ["typescript"] })
    )
...
```ts
    if (!params.yes && stdin.isTTY && stdout.isTTY) {
      yield* launchAddOrgTui({ owner: params.owner, repos: selected, strategy: params.strategy,
