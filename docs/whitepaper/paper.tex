...
making repository context explicit for coding agents. Its flagship route
is version-matched upstream source in the workspace, but the broader
problem is routing: when should context be vendored, linked, fetched,
packed, searched, or exposed through a tool? ingraft is treated here as
one implementation slice rather than the protagonist. The reader's
question is the protagonist: ``what should I use for this job?''
\end{abstract}
...
The timing matters because coding agents are no longer only IDE chat
assistants. Public product documentation now describes agents that run
asynchronously in cloud worktrees, PR queues, desktop apps, mobile
handoffs, and parallel review fleets (OpenAI 2026a; OpenAI 2026b;
Anthropic 2026a; GitHub 2026a; GitHub 2026b; Google 2025). In that
world, the repository becomes the most durable context contract:
project files, source snapshots, route metadata, and agent instructions
survive handoffs more reliably than conversational memory.
...
(\texttt{.cursor/rules}). Codex and Copilot increasingly add cloud,
PR, and app surfaces around the same repository contract. Effect's
git-subtree recommendation is pure vendor leveraging the agent's
tool-mediated capability over the embedded source.
...
\bottomrule\noalign{}
\endlastfoot
Durable source & Agents learn patterns better from source than prose &
...
comments, private access & Route high-centrality packages through
durable source when idiom value is high \\
...
post makes a narrow but important durable-source claim: for an
...
durable source as a candidate route, not enough to make it the global
default.

This is exactly the router opening. If durable source wins for Effect,
...
rule. The durable-source argument is strongest when agents need
...
Recent MCP security research strengthens the authority distinction in
this table. OX Security and Cloud Security Alliance both describe a
2026 MCP STDIO command-execution risk as systemic rather than a narrow
bug (OX Security 2026; Cloud Security Alliance 2026). The practical
router lesson is not ``avoid MCP.'' It is that read-only context
routes, local files, remote retrieval, and executable tool routes must
be different trust classes.
...
docs \\
Durable source & Dependency source in workspace \\
...
Ablations should remove model awareness, privacy constraints, durable
source routing, docs, graph/AST retrieval, stale-context detection, and
...
\item
  ...
  used;
\item
  simpler agentic search dominates retrieval, indexing, and durable
  source routing under realistic budgets.
\end{itemize}
...
ingraft (Brunner 2026) is an open-source CLI for making repository
context explicit for coding agents. Its implemented center of gravity is
\textbf{durable source routing}: adding third-party packages and
repositories into agent-ready reference directories with provenance,
update tooling, and \texttt{AGENTS.md} guidance baked in. Its adjacent
context commands route lighter cases to repo packs, lazy source fetches,
and local search tools.

In the language of \S\ref{sec-strategies}, ingraft implements the
Strategy~1 durable-source route end-to-end, emits the Strategy~4
(instruct) glue that keeps durable source read-only for humans while
remaining visible to agents, and wraps selected Strategy~3/5 routes when
committing or linking source is too heavy. It does not try to be a full routing runtime; the seven
strategies coexist in any non-trivial workflow, and many routes are best
implemented by the tools surveyed in \S\ref{sec-tools}.
...
\item
  Adds chosen upstream repositories as durable source routes under
  \texttt{vendor/}, including committed subtree routes with the same
  revision pin recorded in \texttt{.gitattributes} so PR diffs stay
  focused.
\item
  Supports four durable-source modes: \texttt{subtree} (committed source),
  \texttt{submodule} (gitlink), \texttt{clone-ignore} (local ignored
  clone), and \texttt{cache-link} (ignored symlink to a shared cache
  checkout).
\item
  Detects optional context tools and wraps lighter routes such as
  Repomix-style packs, OpenSrc-style lazy package source lookup, and
  local semantic search.
\item
  Filters durable source routes to omit media, generated directories,
  archives, fixtures, and oversized files when configured.
\item
  Generates \texttt{AGENTS.md} blocks listing durable source routes, the
...
\item
  ...
  size.
\item
  Durable source will help most on idiom-heavy frameworks and least on
...
``make source durable'' and ``use RAG.'' It is a catalog --- seven strategies
...
Brown (2026) & Practitioner durable-source report & Effect source tree
& Locally vendored source gives agents implementation structure, tests,
and examples that docs and web fetches omit & Treat durable source as
...
\bibitem[\citeproctext]{ref-csa2026mcprce}
Cloud Security Alliance. 2026. {``{MCP} {STDIO} Design Flaw Enables
Systemic {AI} Supply Chain {RCE}.''} April 23.
...
\bibitem[\citeproctext]{ref-anthropic2026claudechangelog}
Anthropic. 2026a. {``{Claude Code} Changelog.''} May.
\url{https://code.claude.com/docs/en/changelog}.

\bibitem[\citeproctext]{ref-github2026copilotcli}
GitHub. 2026a. {``{GitHub Copilot CLI} Is Now Generally Available.''}
February 25.
\url{https://github.blog/changelog/2026-02-25-github-copilot-cli-is-now-generally-available/}.

\bibitem[\citeproctext]{ref-github2026multiagent}
GitHub. 2026b. {``Claude and Codex Now Available for {Copilot Business}
\& {Pro} Users.''} February 26.
\url{https://github.blog/changelog/2026-02-26-claude-and-codex-now-available-for-copilot-business-pro-users/}.

\bibitem[\citeproctext]{ref-google2025jules}
Google. 2025. {``Jules, Google's Asynchronous {AI} Coding Agent, Is Out
of Public Beta.''}
\url{https://blog.google/innovation-and-ai/models-and-research/google-labs/jules-now-available/}.

\bibitem[\citeproctext]{ref-openai2026codexmobile}
OpenAI. 2026a. {``Work with {Codex} from Anywhere.''} May 14.
\url{https://openai.com/index/work-with-codex-from-anywhere/}.

\bibitem[\citeproctext]{ref-openai2026codexapp}
OpenAI. 2026b. {``Introducing the {Codex} App.''} February.
\url{https://openai.com/index/introducing-the-codex-app/}.

\bibitem[\citeproctext]{ref-ox2026mcpsupplychain}
OX Security. 2026. {``The Mother of All {AI} Supply Chains: Critical
Systemic Vulnerability at the Core of the {MCP}.''} April 15.
