...
import { handleDashboardKey, type DashboardController } from "../src/keyboard.ts"
import type { VendorTuiSnapshot } from "../src/status.ts"
...
describe("vendor-subtree tui dashboard", () => {
  test("tracks focus and selected task rows independently", () => {
    ...
    expect(visibleTaskRows(selected)).toEqual([
      "  [ ] ADD    effect, @effect/platform -> effect",
      "> [x] UPDATE convex -> convex"
    ])
  })
