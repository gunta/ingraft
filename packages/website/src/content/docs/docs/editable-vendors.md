...
description: Choose the right durable source strategy when upstream source needs local changes.
---

Most durable source context should be treated as read-only reference material.
When you know the project needs to modify upstream source, use a different
workflow on purpose.

## Recommendation

Use a fork-backed `submodule` for durable upstream-source modifications.

```sh
ingraft add your-org/effect --strategy submodule --ref vendor-patches
```

This keeps the parent repository small while making source changes real git
...
It is less clean when the upstream source itself is under active development:

- upstream patches are mixed into the host repository history
...
4. Make source changes inside `vendor/<name>/`.
...
If the patch becomes unnecessary after an upstream release, update the durable
source route back to the upstream repository or a release tag.
...
| Durable source patches, forks, or upstream PRs              | fork-backed `submodule` |
...
When using editable source routes, update agent docs with the ownership rule:
