...
import { githubRepoFromInput, type GitHubRepository } from "./repo.ts"
...
export class GitHubCli extends Effect.Service<GitHubCli>()(
  "vendor-subtree/GitHubCli",
  {
    accessors: true,
...
export const ghRepoCloneFromInput = ({
  cwd,
  input,
  target
}: GitHubCloneFromInputParams) =>
  Option.fromNullable(githubRepoFromInput(input)).pipe(
    Option.match({
      onNone: () => Effect.succeed(Option.none<GitHubCliResult>()),
      onSome: (repo) =>
        ghRepoClone({ cwd, repo, target }).pipe(Effect.map(Option.some))
    })
  )
